...
class AppStore extends Component {
  state = {currentTab: tabsList[0].tabId, searchInput: ''}
  ...
  render() {
    const {currentTab, searchInput} = this.state
    const filteredList = appsList.filter(each => each.category === currentTab)
    const filterBySearch = filteredList.filter(each =>
    ...
    return (
      <div className="container">
        ...
        <ul className="tabs-can">
          {tabsList.map(each => (
            <TabItem
              activeTab={each.tabId === currentTab}
              changeTabOnClick={this.changeTab}
              TabDetails={each}
              key={each.tabId}
            />
          ))}
        </ul>
        <ul className="app-item-con">
          {filterBySearch.map(each => (
